 ...
 * @author adam-
 */
public class VRGCommentDaoTest {
    ...
    public void testUpdateComment() {
        ...
        Comment fromDao = commentDao.getCommentById(comment.getCommentId());
        assertEquals(comment, fromDao);
        Comment updated = commentArray[1];
        updated.setCommentId(comment.getCommentId());
        commentDao.updateComment(updated);
        fromDao = commentDao.getCommentById(comment.getCommentId());
        assertEquals(updated, fromDao);
    }
    ...
    public void testGetAllComments() {
        ...
        assertTrue(commentDao.getAllComments().contains(comment));
        Comment comment2 = commentArray[1];
        commentDao.addComment(comment2, blog.getBlogId());
        assertEquals(2, commentDao.getAllComments().size());
        assertTrue(commentDao.getAllComments().contains(comment));
        assertTrue(commentDao.getAllComments().contains(comment2));
    }
    
    @Test
    public void testGetUnflaggedCommentsByBlogId() {
        Image image = imageArray[0];
        imageDao.addImage(image);
        User user = userArray[0];
        userDao.addUser(user);
        ...
        assertEquals(1, commentDao.getUnflaggedCommentsByBlogId(blog.getBlogId()).size());
        assertTrue(commentDao.getUnflaggedCommentsByBlogId(blog.getBlogId()).contains(comment));
        assertFalse(commentDao.getUnflaggedCommentsByBlogId(blog.getBlogId()).contains(comment2));
    }
    
    @Test
    public void testGetFlaggedCommentsByBlog() {
        Image image = imageArray[0];
        imageDao.addImage(image);
        User user = userArray[0];
        userDao.addUser(user);
        ...
        commentDao.addComment(comment, blog.getBlogId());
        commentDao.addComment(comment2, blog.getBlogId());
        assertEquals(1, commentDao.getFlaggedCommentsByBlogId(blog.getBlogId()).size());
        assertTrue(commentDao.getFlaggedCommentsByBlogId(blog.getBlogId()).contains(comment2));
        assertFalse(commentDao.getFlaggedCommentsByBlogId(blog.getBlogId()).contains(comment));
    }
    
    @Test
    public void testGetAllFlaggedComments() {
        Image image = imageArray[0];
        imageDao.addImage(image);
        User user = userArray[0];
        userDao.addUser(user);
        ...
        assertEquals(1, commentDao.getAllFlaggedComments().size());
        assertTrue(commentDao.getAllFlaggedComments().contains(comment2));
        assertFalse(commentDao.getAllFlaggedComments().contains(comment));
    }
}
